...
class MUCluster[T <: MapUpdateClass[T]]
  (private var _hosts   : Array[(String,Int)],
   ...
   onReceipt        : T => Unit,
   msClient         : MessageServerClient = null) {
  ...
  val self = {
    hosts.zipWithIndex.find { case((host,p),i) =>
  ...
  def init() {
    ...
    hosts.zipWithIndex.foreach { case((host,p),i) =>
  ...
  val queueStatusUpdater = new Thread(run{
    (0 until cluster.hosts.size).foreach{ i =>
      excToOptionWithLog{
        ...
        if (i != cluster.self) {
          (for ( _   <- Option(!ring.dropped.contains(i)) ;
  ...
  def convertPerformers(pHashMap : java.util.HashMap[String, org.json.simple.JSONObject]) = {
    ...
    performers.map(p =>
      Performer(
        ...
        copy    = isTrue(Option(p._2.get("clone").asInstanceOf[String]))
      )
    )(breakOut)
  }
...
case class PerformerPacket(
              ...
              appRun : AppRuntime
              ) extends MapUpdateClass[PerformerPacket] {
