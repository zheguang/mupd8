...
class NodeStatisticsMessenger extends Runnable {
  ...
  val statisticsCollector = RuntimeProvider.getStatisticsCollector()
  val msClient = RuntimeProvider.getMessageServerClient()

  def run() = {
    var hotKeys: Array[String] = null
    while (true) {
      var statsReport = new NodeStatisticsReport("localhost", System.currentTimeMillis(), java.lang.Double.parseDouble(statisticsCollector.getNodeLoadAvg()), hotKeys)
